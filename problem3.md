...
The site features multiple React components interacting with a _Golang backend API_ and _PostgreSQL_ database provisioned and deployed on _Heroku_. SMS messages are generated through the third party _Twilio API_ and the chat functionality is an integration of the _Stream Chat API_ (separate _nodeJS API_ with a _PSQL database_ deployed with AWS endpoints using _Serverless AWS lambda and S3_ [nodeJS Chat Github Repository Here](https://github.com/rafischer1/stream-challenge-be)).
